...
public class TargetDetectionController {
    ...
    private TargetDetectionFrame      frame;
    private String		    selectedCameraURL;
    private Thread		    imageThread;
    private BufferedImage 	    selectedImage;
    private TargetDetectionDetector	detector;
    ...
    // CONSTRUCTOR
    public TargetDetectionController(TargetDetectionFrame frame) {
        this.frame = frame;
	...
	valThresh = 0;
        ...
        frame.getChooseCameraSourceButton().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				TargetDetectionController.this.chooseCameraSourceAction();
			}
		});
        ...
        frame.getChooseImageButton().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
			    ...
			    int returnVal = chooser.showOpenDialog(TargetDetectionController.this.frame);
		...
		this.imageThread = new Thread(new Runnable() {
			...
			public void run() {
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
						BufferedImage out = TargetDetectionController.this.processImage(selectedImage);
                            			TargetDetectionController.this.getFrame().getCenterImage().setImage(out);
					}
				});
			}
		});
    ...
    protected void startCamera() {
		...
		this.imageThread = new Thread(new Runnable() {
			@Override
            public void run() {
                ImageSourceFormat fmt = TargetDetectionController.this.selectedImageSource.getCurrentFormat();
                while (true) {
                    // get buffer with image data from next frame
                    byte buf[] = TargetDetectionController.this.selectedImageSource.getFrame().data;
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        @Override
                        public void run() {
                            BufferedImage out = TargetDetectionController.this.processImage(im);
                            TargetDetectionController.this.getFrame().getCenterImage().setImage(out);
                        }
                    });
                }
